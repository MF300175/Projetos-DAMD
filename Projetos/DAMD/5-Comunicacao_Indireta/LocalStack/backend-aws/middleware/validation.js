...
function validateTask(taskData) {
  ...
  // client_id é opcional - usar ID do dispositivo se não fornecido
  if (!taskData.client_id || taskData.client_id.trim() === '') {
    // Gerar um client_id baseado no ID da tarefa se não fornecido
    taskData.client_id = taskData.id || `client_${Date.now()}`;
  }
